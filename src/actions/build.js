...
module.exports = options => {
  ...
  let change = options.change || null
  let metadata
  let settings = buildHelpers.readSettings(settingsFileName)
  ...
  return sequence([
    ...
    () =>
      (clear || change === 'src') &&
      buildES('es6', settings.platformSettings.esEnv, options.types) &&
      buildHelpers.bundleEs6App(targetDir, metadata),
    () =>
      (clear || change === 'src') &&
      buildES('es5', settings.platformSettings.esEnv, options.types) &&
